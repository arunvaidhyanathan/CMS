...
-- Updates in DB on 05/25/2025
------------------------------

-- Add the missing workflow_instance_key column to the cases table
ALTER TABLE cms_workflow.cases 
ADD COLUMN workflow_instance_key BIGINT;


CREATE INDEX idx_cases_workflow_instance_key ON cms_workflow.cases(workflow_instance_key);
...
AND column_name = 'workflow_instance_key';


ALTER TABLE cms_workflow.work_items 
ADD COLUMN task_key BIGINT;

-- Create an index on task_key for better performance
CREATE INDEX idx_work_items_task_key ON cms_workflow.work_items(task_key);
